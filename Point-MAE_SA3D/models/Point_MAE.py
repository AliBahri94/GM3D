...
class Group(nn.Module):  # FPS + KNN
    ...
    def forward(self, xyz):
        ...
        # fps the centers out
        center = miscc.fps(xyz, self.num_group) # B G 3
...
class TransformerEncoder_CLS(nn.Module):
    def __init__(self, embed_dim=768, depth=4, num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0.):
        super().__init__()
...
class PointTransformer(nn.Module):
    def __init__(self, config, **kwargs):
        ...
        #self.cls_token = nn.Parameter(torch.zeros(1, 1, 768))                   ########################## baraye load hard_patches avaz shode
    ...
    def forward(self, pts):
        ...
        #cls_tokens = self.cls_token[..., :384].expand(group_input_tokens.size(0), -1, -1)
...
class Classifier(nn.Module):
    def __init__(self):
        ...
        self.num_heads = 1
        dpr = [x.item() for x in torch.linspace(0, self.drop_path_rate, self.depth)]
        self.blocks = TransformerEncoder_CLS(
            embed_dim=self.trans_dim,
            depth=self.depth,
            drop_path_rate=dpr,
            num_heads=self.num_heads,
    ...
    """def forward(self, x, center):
        ...
        cls_pos = self.cls_pos.expand(x.size(0), -1, -1)
        pos = self.pos_embed(center)
        pos = torch.cat((cls_pos, pos), dim=1)
